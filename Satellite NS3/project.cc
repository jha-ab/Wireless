 ...
 * \ingroup satellite
 *
 *         This is an example of Random Access usage in satellite network. This
 *         example uses only the CRDSA model regardless of the number of instances
 *         parameter. CRA(Constant Rate Allocation) is disabled in this example. The example allows to
 ...
 *
 *         The script is using CBR(Constant Bit Rate) application in user defined scenario,
 ...
 *
 *         execute command -> ./waf --run "project --PrintHelp"
 */
...
main (int argc, char *argv[])
{
 ...
 LogComponentEnable ("SatBeamScheduler", LOG_LEVEL_INFO);
  ...
  auto sh = CreateObject<SimulationHelper> ("example-random-access-crdsa");

//Enable Packet Capture
  Config::SetDefault ("ns3::SatHelper::PacketTraceEnabled", BooleanValue (true));

//Set TCP Flavor to NewReno
